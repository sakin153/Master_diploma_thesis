...
## 📊 Общий прогресс: 18/22 задачи завершены (82%)

### Статус по Milestones:
- ✅ **Milestone 1**: Структуры данных и утилиты — **4/4 (100%)**
- ✅ **Milestone 2**: Построение графа (SceneGraphBuilder) — **5/5 (100%)**
- ✅ **Milestone 3**: Иерархическое размещение (HierarchicalPlacer) — **7/7 (100%)**
- ✅ **Milestone 4**: Сборка и интеграция — **2/2 (100%)**
- ⏳ **Milestone 5**: Тестирование — **0/4 (0%)**

### Ключевые достижения:
- ✅ Новая архитектура полностью реализована в `project/scene_planner.py`
- ✅ Граф сцены с иерархией объектов (level 0/1/2)
- ✅ Итеративное размещение с обновлением world_state
- ✅ Контекст будущих объектов при размещении anchors
- ✅ Батчинг объектов по ≤6 с зависимостями (depends_on)
- ✅ Fallback на эвристический граф при ошибках LLM
- ✅ Совместимость с существующим пайплайном (validate_and_repair_layout)

### Что осталось:
- ⏳ Тестирование базовых сценариев (Milestone 5)
...
- [x] `world_state_add(world_state, node, placed_results)` → обновляет `placed` и `remaining` (реализовано как `_ws_add`)
- [x] `world_state_summary(world_state, max_items=10)` → строка для промпта (реализовано как `_ws_summary`)
- [x] `remaining_summary(world_state)` → строка "в сцене ещё будет: 4x Chair, 2x Plate" (реализовано как `_ws_remaining_summary`)
- [x] `find_placed(world_state, object_id)` → dict или None (реализовано как `_ws_find`)
...
- [x] Вызвать LLM с `GRAPH_HIERARCHY_PROMPT` (реализовано в `_build_scene_graph`)
- [x] Валидировать через `SceneGraphValidator` (реализовано как `_validate_graph`)
...
- [x] Вызвать LLM с `ANCHOR_RELATIONS_PROMPT` (реализовано в `_place_anchors_by_llm`)
...
- [x] Вызвать `build_hierarchy()` → получить nodes (реализовано в `_build_scene_graph`)
- [x] Вызвать `build_anchor_rels()` → получить relations + anchor_positions (реализовано в `_place_anchors_by_llm`)
- [x] Собрать финальный SceneGraph
- [x] Вычислить `metadata` (total_objects, max_level)
- [x] Логировать граф в читаемом виде через `_log_graph`:
  ```
...
- [x] Описание группы (тип, количество, суммарный батч) (реализовано как `_BATCH_PLACEMENT_PROMPT`)
...
- [x] Найти parent в world_state (если level > 0) (реализовано в `_place_single_node`)
- [x] Сформировать `relationship_description` из node["relationship"] (реализовано как `_rel_description`)
- [x] Вызвать LLM с `SINGLE_PLACEMENT_PROMPT`
- [x] Валидировать ответ: координаты в границах комнаты (реализовано как `_validate_pos`)
- [x] При ошибке — retry + fallback позиция (реализовано как `_fallback_pos`)
...
- [x] Отсортировать узлы группы по `place_order`, затем по `depends_on` (зависимые — позже) (реализовано в `_place_all`)
- [x] Итерировать по узлам в порядке сортировки:
  - `instances == 1` → вызвать `_place_single(node, ...)` (реализовано как `_place_single_node`)
  - `instances > 1` → вызвать `_place_node_batch(node, ...)` (реализовано как `_place_batch_node`)
- [x] После каждого узла обновить world_state — следующий узел видит уже размещённых (реализовано через `_register_results`)
...
- [x] Разбить `instances` на суб-батчи по ≤ 6 (реализовано в `_place_batch_node`)
- [x] Для каждого суб-батча:
  ...
  - [x] Вызвать LLM с `GROUP_PLACEMENT_PROMPT` (реализовано как `_BATCH_PLACEMENT_PROMPT`)
  - [x] Валидировать ответ (правильное число объектов, координаты в комнате)
  - [x] При ошибке — retry с feedback
  - [x] Обновить world_state через `world_state_add()` (реализовано через `_register_results`)
- [x] Логировать: `[placer] Group chair_group: placed 4/4 (1 batch)`

### TASK-14: `HierarchicalPlacer.place_all` ✅
**Файл:** `project/scene_planner.py`

- [x] Инициализировать world_state из anchor_positions (результат Milestone 2) (реализовано в `_place_all`)
- [x] Добавить anchors с их позициями в world_state
- [x] Итерировать по уровням: 0 → 1 → 2 → ...
- [x] На каждом уровне: итерировать по узлам
  - [x] `instances == 1` → `_place_single()` (реализовано как `_place_single_node`)
  - [x] `instances > 1` → `_place_group()` (реализовано как `_place_batch_node`)
- [x] После каждого размещения — обновление через `_register_results`
- [x] Возвращать `world_state["placed"]`

### TASK-15: Валидация координат ✅
**Файл:** `project/scene_planner.py`

- [x] `_validate_placement_result(result, room_half_size, node_size)` → `(bool, str)` (реализовано как `_validate_pos`)
...
- [x] `_fallback_position(node, parent, world_state, room_half_size)` → dict (реализовано как `_fallback_pos`)
...
- [x] Заменить вызов `_generate_plan_in_batches` на:
  1. `SceneGraphBuilder.build()` → граф (реализовано как `_build_scene_graph`)
  2. `HierarchicalPlacer.place_all()` → placed list (реализовано как `_place_all`)
...
### TASK-19: Удалить устаревший код ⚠️
**Файл:** `docs/scene_planner.py`

**ПРИМЕЧАНИЕ:** Старая реализация сохранена в `docs/scene_planner.py` для справки. Новая реализация в `project/scene_planner.py` полностью заменяет старую архитектуру.

Старые компоненты (больше не используются):
- ❌ `_generate_plan()` — заменён на `_build_scene_graph` + `_place_all`
- ❌ `_generate_plan_in_batches()` — заменён на иерархическое размещение
- ❌ `_determine_priorities()` — заменён на `_build_scene_graph`
- ❌ `_group_by_anchor()` — заменён на `_collect_groups`
- ❌ `_PLACEMENT_PRIORITY_PROMPT` — больше не нужен
- ❌ `_SEMANTIC_PLAN_PROMPT` — заменён на `_GRAPH_HIERARCHY_PROMPT` + `_SINGLE_PLACEMENT_PROMPT` + `_BATCH_PLACEMENT_PROMPT`
- ❌ `_DistanceResolver` — больше не нужен (размещение через LLM)
- ❌ `_OrientationResolver` — больше не нужен (размещение через LLM)
- ❌ `_PlacementExecutor` — заменён на `_merge_to_output`
- ❌ `_SchemaValidator` — заменён на `_validate_graph`
...
**ПРИМЕЧАНИЕ:** Тестирование требует запуска реальных сценариев с LLM и MuJoCo. Рекомендуется выполнить после интеграции в основной пайплайн.
